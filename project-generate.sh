...
if [[ $# -lt 2 ]];
then
  echo "Usage: project-generate.sh PROJECT_NAME VHDL_PATH [VHDEPS_INCLUDE_DIRS]..."
...
cp --recursive --no-dereference "$SCRIPT_DIR/project-skeleton" "$PROJ_NAME"

# copy vhdl dir into project/design
cp --recursive --no-dereference "$VHDL_PATH" "$PROJ_NAME/design/"

cd "$PROJ_NAME"

# re-link the fletcher-aws directory symlink
rm "fletcher-aws"
ln -s "$SCRIPT_DIR" "fletcher-aws"
