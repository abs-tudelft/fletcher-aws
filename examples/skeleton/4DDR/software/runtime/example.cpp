...
extern "C" void test_main(uint32_t *exit_code) {
  ...
  printf("example.cpp test_main simulation runtime entry point\n");
  ...
  if(rb_base) {
    ...
    "of the test recordbatches generated by example.py, excluding the number+extension suffix (000.rb).\n");
    printf("For example: `export TEST_RECORDBATCH_BASE=~/workspaces/example/gen/rematch`.\n");
    exit_code = 0;
    return;
  }
  
  char *argv[] = {(char*)"example", rb_base, (char*)"16", (char*)"16"};
  if (example_main(4, argv) == 0) {
    *exit_code = 1;
  } else {
    *exit_code = 0;
  }
